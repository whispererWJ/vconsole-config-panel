...
import { generateHtmlWithId, initPanelById, refreshPanelData } from './helper';
// 导出加密工具
export * from './crypto';
 ...
 * 创建vConsole配置面板插件
 */
function VConsoleConfigPanel(options) {
    ...
    function tryInitPanel() {
        ...
        if (!panel) {
            ...
            panel = document.querySelector(`[data-panel-id="${panelId}"]`);
        }
        if (!panel) {
    ...
    plugin.on('show', () => {
        ...
        if (panelId) {
            ...
            if (panel) {
                ...
                const panelByData = document.querySelector(`[data-panel-id="${panelId}"]`);
                if (panelByData) {
...
// 导出工具函数
export { showToast, refreshPanelData as refreshPanel, encryptDataWithConfig, decryptDataWithConfig, copyToClipboard, collectConfigData } from './helper';
